package com.example.lkmagneticrobot.util.usbfpv;
...
public class FPVVideoClient {
    ...
    private Runnable runnable = new Runnable() {
        ...
        public void run() {
            while (circularByteBuffer.available() > this.buffer.length) {
                int count = circularByteBuffer.get(this.buffer);
                if (count <= 4) {
                    return;
                }
                ...
                try {
                    ...
                    int end;
                    byte[] h264s;
                    for (int start = BusinessUtils.Find(buffer, bufferCount, 0,H264Header); start != -1; start = end) {
                        end = BusinessUtils.Find(buffer, bufferCount, start +H264Header.length,H264Header);
                        if (end != -1) {
                            h264s = Arrays.copyOfRange(buffer, start, end);
                           decodeH264(h264s, h264s.length);
                        }
                    }

                    end = BusinessUtils.FindR(buffer, bufferCount,H264Header);
    ...
    public boolean startRecord(@Nullable String folder, @Nullable String fileName) {
        if (!this.isPlaying) {
            ...
            if (TextUtils.isEmpty(folder)) {
                tempFolder = Environment.getExternalStorageDirectory().getAbsolutePath() + "/FUAV/video/";
            }
    ...
    public void captureSnapshot(@Nullable String folder, @Nullable String fileName) {
        if (this.isPlaying) {
            String tempFolder = folder;
            if (TextUtils.isEmpty(folder)) {
                tempFolder = Environment.getExternalStorageDirectory().getAbsolutePath() + "/FUAV/video/";
...
//            byte[] i420 = new byte[640 * 360 * 3 / 2];
    ...
    public void startPlayback() {
        ...
        this.rendererThread = new RendererThread();
                        ...
                        if (frame != null) {
                            synchronized (FPVVideoClient.this.surfaceLock) {
                                if (FPVVideoClient.this.delegate != null) {
                                   delegate.renderI420(frame.frame, frame.width, frame.height);
                                }
                            }

                            if (FPVVideoClient.this.avWriter.isOpened()) {
                               avWriter.putFrame420(frame.frame, frame.width, frame.height);
//                                avWriter.putFrame420(frame.frame, 1920, 1200);
                            }
                        }
